...
subroutine GetMask(NCols,NRows,jCol_outlet,iRow_outlet,NoData_Value,FDR, &
    ...
    integer :: i,j,iiA,iiB,jjA,jjB,step_count
    integer :: max_steps
    logical,external :: InBasin

    MaskOut=NoData_Value
    MaskOut(jCol_outlet,iRow_outlet)=1
    max_steps = NCols * NRows ! Maximum possible steps to prevent infinite loop
        ...
        do j = 0, NCols-1
            ...
            do while(InBasin(jjA,iiA) .eqv. .true.)
                ...
                if(step_count > max_steps)then
                    ! Prevent infinite loop - exit if too many steps
                    exit
                end if
